package com.example.walk10.accounthelper
...
class AccountHelper (act:MainActivity) {
        ...
        if (email.isNotEmpty() && password.isNotEmpty()) {
            act.mAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener{task->
                if(task.isSuccessful){
                    ...
                    Toast.makeText(act, act.resources.getString(R.string.reg_error), Toast.LENGTH_LONG).show()
                }
            }
    ...
    private fun sendEmailVerification(user:FirebaseUser){
        user.sendEmailVerification().addOnCompleteListener { task->
            if(task.isSuccessful){
                Toast.makeText(act, act.resources.getString(R.string.send_verification_email), Toast.LENGTH_LONG).show()
            }
            else{
